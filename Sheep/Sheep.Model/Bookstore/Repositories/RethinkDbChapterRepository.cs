...
namespace Sheep.Model.Bookstore.Repositories
{
    ...
    public class RethinkDbChapterRepository : IChapterRepository, IClearable
    {
        ...
        {
            return R.Table(s_ChapterTable).GetAll(R.Args(chapterIds.ToArray())).Skip(0).Limit(100000).RunResult<List<Chapter>>(_conn);
        }

        /// <inheritdoc />
        public Task<List<Chapter>> GetChaptersAsync(List<string> chapterIds)
        {
            return R.Table(s_ChapterTable).GetAll(R.Args(chapterIds.ToArray())).Skip(0).Limit(100000).RunResultAsync<List<Chapter>>(_conn);
        }
        ...
        public List<Chapter> FindChapters(string bookId, int? volumeNumber, string contentFilter, string orderBy, bool? descending, int? skip, int? limit)
        {
            ...
            return queryOrder.Skip(skip ?? 0).Limit(limit ?? 100000).RunResult<List<Chapter>>(_conn);
        }
        ...
        public Task<List<Chapter>> FindChaptersAsync(string bookId, int? volumeNumber, string contentFilter, string orderBy, bool? descending, int? skip, int? limit)
        {
            ...
            return queryOrder.Skip(skip ?? 0).Limit(limit ?? 100000).RunResultAsync<List<Chapter>>(_conn);
        }
        ...
        public void DeleteChapter(string chapterId)
        {
            ...
            R.Table(s_ParagraphAnnotationTable).GetAll(chapterId).OptArg("index", "ChapterId").Delete().RunResult(_conn).AssertNoErrors();
        }
        ...
        public async Task DeleteChapterAsync(string chapterId)
        {
            ...
            (await R.Table(s_ParagraphAnnotationTable).GetAll(chapterId).OptArg("index", "ChapterId").RunResultAsync(_conn)).AssertNoErrors();
        }
