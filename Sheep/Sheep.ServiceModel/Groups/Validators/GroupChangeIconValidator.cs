            ...
            RuleSet(ApplyTo.Put, () =>
                                 {
                                     ...
                                     RuleFor(x => x.SourceIconUrl).Must(url => url.GetImageUrlExtension().IsImageExtension()).WithMessage(x => string.Format(Resources.SourceIconUrlMismatch)).When(x => !x.SourceIconUrl.IsNullOrEmpty());
                                 });
        }
    }
}