...
namespace Sheep.ServiceModel.Accounts.Validators
{
    ...
    public class AccountChangeAvatarValidator : AbstractValidator<AccountChangeAvatar>
    {
        ...
        public AccountChangeAvatarValidator()
        {
            RuleSet(ApplyTo.Put, () =>
                                 {
                                     RuleFor(x => x.SourceAvatarUrl).Must(url => url.GetImageUrlExtension().IsImageExtension()).WithMessage(x => string.Format(Resources.SourceAvatarUrlMismatch)).When(x => !x.SourceAvatarUrl.IsNullOrEmpty());
                                 });
        }
    }
}