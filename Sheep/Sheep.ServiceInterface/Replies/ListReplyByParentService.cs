...
namespace Sheep.ServiceInterface.Replies
{
    ...
    public class ListReplyByParentService : Service
    {
        ...
        ///     相关的日志记录器。
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(ListReplyByParentService));
        ...
        ///     获取及设置列举一组回复的校验器。
        /// </summary>
        public IValidator<ReplyListByParent> ReplyListByParentValidator { get; set; }
        ...
        public async Task<object> Get(ReplyListByParent request)
        {
            ...
            //}
            var existingReplies = await ReplyRepo.FindRepliesByParentAsync(request.ParentId, request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), "审核通过", request.OrderBy, request.Descending, request.Skip, request.Limit);
