...
namespace Sheep.ServiceInterface.ChapterReads
{
    ...
    ///     根据上级列举一组阅读信息服务接口。
    /// </summary>
    public class ListChapterReadByChapterService : Service
    {
        #region 静态变量
        ...
        ///     相关的日志记录器。
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(ListChapterReadByChapterService));
        ...
        ///     获取及设置列举一组阅读的校验器。
        /// </summary>
        public IValidator<ChapterReadListByChapter> ChapterReadListByChapterValidator { get; set; }
        ...
        public async Task<object> Get(ChapterReadListByChapter request)
        {
            //if (HostContext.GlobalRequestFilters == null || !HostContext.GlobalRequestFilters.Contains(ValidationFilters.RequestFilter))
            //{
            //    ChapterReadListByChapterValidator.ValidateAndThrow(request, ApplyTo.Get);
            //}
            var currentUserId = GetSession().UserAuthId.ToInt(0);
            var existingChapterReads = await ChapterReadRepo.FindChapterReadsByChapterAsync(request.ChapterId, request.IsMine.HasValue && request.IsMine.Value ? currentUserId : (int?) null, request.CreatedSince?.FromUnixTime(), request.OrderBy, request.Descending, request.Skip, request.Limit);
