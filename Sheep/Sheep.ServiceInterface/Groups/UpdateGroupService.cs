...
namespace Sheep.ServiceInterface.Groups
{
    ...
    public class UpdateGroupService : Service
    {
        ...
        public async Task<object> Put(GroupUpdate request)
        {
            ...
            {
                var ownerUserAuth = authRepo.GetUserAuth(group.OwnerId.ToString());
        ...
        public Group MapToGroup(Group existingGroup, GroupUpdate request)
        {
            ...
            if (!request.JoinMode.IsNullOrEmpty())
            {
                newGroup.JoinMode = request.JoinMode;
            }
            if (request.IsPublic.HasValue)
            {
                newGroup.IsPublic = request.IsPublic.Value;
            }
            if (request.EnableMessages.HasValue)
            {
                newGroup.EnableMessages = request.EnableMessages.Value;
            }
            return newGroup;
        }
            ...
            var groupDto = new GroupDto
                           {
                               ...
                               Description = group.Meta.GetValueOrDefault("Description"),
