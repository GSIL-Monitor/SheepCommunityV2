...
namespace Sheep.ServiceInterface.PostBlocks
{
    ...
    /// </summary>
    [CompressResponse]
    public class ListPostBlockService : Service
    {
        #region 静态变量
        ...
        ///     相关的日志记录器。
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(ListPostBlockService));
        ...
        ///     获取及设置列举一组帖子屏蔽的校验器。
        /// </summary>
        public IValidator<PostBlockList> PostBlockListValidator { get; set; }
        ...
        public async Task<object> Get(PostBlockList request)
        {
            if (!IsAuthenticated)
            {
            ...
            //{
            //    PostBlockListValidator.ValidateAndThrow(request, ApplyTo.Get);
            //}
            var existingPostBlocks = await PostBlockRepo.FindPostBlocksAsync(request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), request.OrderBy, request.Descending, request.Skip, request.Limit);
