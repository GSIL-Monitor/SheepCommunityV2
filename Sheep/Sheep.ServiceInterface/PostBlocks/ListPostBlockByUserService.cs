...
namespace Sheep.ServiceInterface.PostBlocks
{
    ...
    /// </summary>
    [CompressResponse]
    public class ListPostBlockByBlockerService : Service
    {
        #region 静态变量
        ...
        ///     相关的日志记录器。
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(ListPostBlockByBlockerService));
        ...
        ///     获取及设置列举一组帖子屏蔽的校验器。
        /// </summary>
        public IValidator<PostBlockListByBlocker> PostBlockListByBlockerValidator { get; set; }
        ...
        public async Task<object> Get(PostBlockListByBlocker request)
        {
            //if (HostContext.GlobalRequestFilters == null || !HostContext.GlobalRequestFilters.Contains(ValidationFilters.RequestFilter))
            //{
            //    PostBlockListByBlockerValidator.ValidateAndThrow(request, ApplyTo.Get);
            //}
            var existingPostBlocks = await PostBlockRepo.FindPostBlocksByBlockerAsync(request.BlockerId, request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), request.OrderBy, request.Descending, request.Skip, request.Limit);
