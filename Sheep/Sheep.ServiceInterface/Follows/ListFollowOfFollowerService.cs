...
namespace Sheep.ServiceInterface.Follows
{
    ...
    public class ListFollowOfFollowerService : Service
    {
        ...
        public async Task<object> Get(FollowListOfFollower request)
        {
            ...
            //}
            var existingFollows = await FollowRepo.FindFollowsByOwnerAsync(request.OwnerId, request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), request.OrderBy, request.Descending, request.Skip, request.Limit);
