...
namespace Sheep.ServiceInterface.Votes
{
    ...
    /// </summary>
    [CompressResponse]
    public class ListVoteByUserService : Service
    {
        #region 静态变量
        ...
        ///     相关的日志记录器。
        /// </summary>
        protected static readonly ILog Log = LogManager.GetLogger(typeof(ListVoteByUserService));
        ...
        ///     获取及设置列举一组投票的校验器。
        /// </summary>
        public IValidator<VoteListByUser> VoteListByUserValidator { get; set; }
        ...
        public async Task<object> Get(VoteListByUser request)
        {
            ...
            //}
            var existingVotes = await VoteRepo.FindVotesByUserAsync(request.UserId, request.ParentType, request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), request.OrderBy, request.Descending, request.Skip, request.Limit);
