...
namespace Sheep.ServiceInterface.Users
{
    ...
    public class ListUserByIdsService : Service
    {
        ...
        public async Task<object> Get(UserListByIds request)
        {
            ...
            //}
            var existingUserAuths = await ((IUserAuthRepositoryExtended) AuthRepo).FindUserAuthsAsync(request.UserIds.Select(userId => userId.ToString()).ToList(), request.CreatedSince?.FromUnixTime(), request.ModifiedSince?.FromUnixTime(), request.LockedSince?.FromUnixTime(), null, request.OrderBy, request.Descending, request.Skip, request.Limit);
